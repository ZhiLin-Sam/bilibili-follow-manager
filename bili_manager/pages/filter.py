...
class FilterPage(BasePage):
    ...
    def _build(self, parent: ttk.Frame) -> None:
        ...
        ttk.Label(f, text="规则过滤 & 深度探测", style="Header.TLabel").pack(
            anchor="w", pady=(0, 5)
        )
        ttk.Label(
            f,
            text="应用预设规则 + 自定义规则自动判定，深度探测拉取空间数据",
        ...
        ttk.Button(
            btn_row, text="运行规则过滤", style="Accent.TButton", command=self._apply_rules
        ).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Button(btn_row, text="深度探测 (活跃度分析)", command=self._start_probe).pack(
            side=tk.LEFT, padx=(0, 8)
        )
        ttk.Button(btn_row, text="⏹ 停止", command=self._stop_op, state=tk.DISABLED).pack(
            side=tk.LEFT, padx=(0, 8)
        )
        ...
        ttk.Label(f, textvariable=self.result_var, style="Mono.TLabel").pack(
            anchor="w", pady=(10, 0)
        )
    ...
    def _start_probe(self):
        ...
        conn = database.get_conn()
        rows = conn.execute(
            "SELECT f.mid FROM follows f LEFT JOIN probes p ON f.mid = p.mid WHERE p.mid IS NULL OR p.spacesta = -999"
        ).fetchall()
        conn.close()
        ...
        def _run():
            try:
                ...
                results = batch_probe(
                    client, uids, concurrency=5, batch_delay=3.0, progress_callback=progress
                )
                if self._stop_requested:
