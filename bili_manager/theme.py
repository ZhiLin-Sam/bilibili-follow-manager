...
def apply_theme(root: tk.Tk) -> ttk.Style:
    ...
    style.configure(".", background=BG0, foreground=FG0, font=FONT, borderwidth=0, troughcolor=BG1)
    ...
    style.configure(
        "Accent.TButton", background=ACCENT, foreground="#ffffff", font=FONT_BOLD, padding=(14, 5)
    )
    style.map(
        "Accent.TButton",
    ...
    style.configure(
        "Danger.TButton", background="#5a1a1a", foreground=RED, font=FONT_BOLD, padding=(14, 5)
    )
    style.map(
        "Danger.TButton",
    ...
    style.map("Sidebar.TButton", background=[("active", BG2), ("pressed", ACCENT)])
    ...
    # ── 输入框 ──
    style.configure(
        "TEntry", fieldbackground=BG2, foreground=FG0, font=FONT, borderwidth=1, insertcolor=FG0
    )
    style.map("TEntry", fieldbackground=[("disabled", BG1)])

    # ── 进度条 ──
    style.configure("TProgressbar", background=ACCENT, troughcolor=BG2, borderwidth=0, thickness=6)

    # ── 标签页 ──
    style.configure("TNotebook", background=BG0, borderwidth=0)
    style.configure(
        "TNotebook.Tab", background=BG1, foreground=FG0, font=FONT, borderwidth=0, padding=(16, 6)
    )
    style.map(
        "TNotebook.Tab",
    ...
    # ── 滚动条 ──
    style.configure(
        "TScrollbar", background=BG2, troughcolor=BG1, borderwidth=0, arrowsize=14, arrowcolor=FG1
    )
